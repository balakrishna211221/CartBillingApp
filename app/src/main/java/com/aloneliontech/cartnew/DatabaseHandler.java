...
public class DatabaseHandler extends SQLiteOpenHelper {
    public static final String CART_TABLE = "Akhilacart";
    ...
    public boolean setCart(HashMap<String, String> map, Integer Qty) {
        ...
        if (isInCart(map.get(VARIENT_ID))) {
            db.execSQL("update " + CART_TABLE + " set " + COLUMN_QTY + " = '" + Qty + /*"',"+ COLUMN_AreaId+"='"+map.get(COLUMN_AreaId)+*/"'where " + VARIENT_ID + "=" + map.get(VARIENT_ID));
            return false;
        } else {
    ...
    public String getInCartItemprice(String id) {
        if (isInCart(id)) {
            db = getReadableDatabase();
            String qry = "Select *  from " + CART_TABLE + " where " + VARIENT_ID + " = " + id;
            Cursor cursor = db.rawQuery(qry, null);
